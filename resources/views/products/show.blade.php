            ...
            <div class="grid lg:grid-cols-2 gap-12">
                ...
                <div>
                    ...
                    <div class="space-y-4">
                        @if($product->stock > 0)
                                ...
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <button type="button" onclick="decreaseQty()" 
                                                class="w-10 h-10 rounded-xl bg-white border border-gray-200 hover:bg-gray-100 flex items-center justify-center transition-colors">
                                            <span class="text-xl text-gray-600">−</span>
                                        </button>
                                        <input type="number" id="product-qty" value="1" min="1" max="{{ $product->stock }}"
                                               class="w-16 h-10 text-center border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent">
                                        <button type="button" onclick="increaseQty({{ $product->stock }})" 
                                                class="w-10 h-10 rounded-xl bg-white border border-gray-200 hover:bg-gray-100 flex items-center justify-center transition-colors">
                                            <span class="text-xl text-gray-600">+</span>
                                        </button>
                                    </div>
                                    <span class="text-sm text-gray-500">(Maks: {{ $product->stock }})</span>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <button onclick="addToCartDetail({{ $product->id }})" 
                                        class="w-full inline-flex items-center justify-center px-8 py-4 bg-primary-500 hover:bg-primary-600 text-white font-bold rounded-full transform hover:scale-105 transition-all shadow-lg">
                                ...
                                <!-- WhatsApp Button -->
                                <a href="https://wa.me/6285201060671?text={{ urlencode('Halo, saya tertarik dengan produk ' . $product->name . ' (' . $product->formatted_price . ')') }}" target="_blank" class="w-full inline-flex items-center justify-center px-8 py-4 bg-green-500 text-white font-bold rounded-full hover:bg-green-600 transform hover:scale-105 transition-all shadow-lg">
                        ...
                        @else
                            <div class="w-full inline-flex items-center justify-center px-8 py-4 bg-gray-300 text-gray-500 font-bold rounded-full cursor-not-allowed">
                                Stok Habis
                            </div>
                        @endif
                        <p class="text-center text-gray-500 text-sm">
                            Tambahkan ke keranjang untuk checkout banyak produk sekaligus
                        </p>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function showNotification(message, type) {
        ...
        notification.className = `fixed top-20 right-4 z-50 px-6 py-4 rounded-xl shadow-lg transform transition-all duration-300 translate-x-full ${
